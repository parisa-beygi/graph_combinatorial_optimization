...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            assert problem.NAME == "coloring"
            # Todo: think of another step_context_dim
            step_context_dim = graph_size * embedding_dim
            node_dim = 64
    ...
    def forward(self, input, return_pi=False):
        ...
        #_log_p = [1024, 20, 20], pi = [1024, 20]
        # embeddings: output of encoder part
    ...
    def _calc_log_likelihood(self, _log_p, a, mask):
        ...
        # _log_p = [1024, 20, 20], a = [1024, 20] ==> log_p = [1024, 20]
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            # log_p = [1024, 1, 20], glimpse = [1024, 1, 128]
            ...
            # appending [1024, 20] to outputs
            # appending [1024] to sequences
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        # graph_embed = (batch_size, embed_dim)
        # fixed context = (batch_size, 1, embed_dim) to make broadcastable with parallel timesteps
        fixed_context = self.project_fixed_context(graph_embed)[:, None, :]

        # The projection of the node embeddings for the attention is calculated once up front
        # self.project_node_embeddings = [1024, 1, 20, 384]
        # glimpse_key_fixed.shape = torch.Size([1024, 1, 20, 128])
        glimpse_key_fixed, glimpse_val_fixed, logit_key_fixed = \
            self.project_node_embeddings(embeddings[:, None, :, :]).chunk(3, dim=-1)

        # No need to rearrange key for logit as there is a single head
        # ([8, 1024, 1, 20, 16], [8, 1024, 1, 20, 16], [1024, 1, 20, 128])
        ...
        # AttentionModelFixed =
        # (embeddings: [1024, 20, 128],
        # context_node_projected: [1024, 1, 128]
        # glimpse_key: [8, 1024, 1, 20, 16],
        # glimpse_val: [8, 1024, 1, 20, 16],
        # logit_key: [1024, 1, 20, 128])
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask):
        ...
        # glimpse_Q = [8, 1024, 1, 1, 16]
        glimpse_Q = query.view(batch_size, num_steps, self.n_heads, 1, key_size).permute(2, 0, 1, 3, 4)

        # Batch matrix multiplication to compute compatibilities (n_heads, batch_size, num_steps, graph_size)
        # compatibility = [8, 1024, 1, 1, 20]
        ...
        # input to self.project_out is of [1024, 1, 1, 128]
        # glimpse = [1024, 1, 1, 128]
        set_trace()
        ...
        # logits = 'compatibility' [1024, 1, 20]
