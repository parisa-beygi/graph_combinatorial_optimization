...
def create_dataset(graph_size, path, samples):
    # set_trace()
    Ma_list = []
    ...
    while z in range(samples):
        ...
        Ma = np.zeros((graph_size, graph_size))
        ...
        Ma = gen_matrix(graph_size, p_connected)
        ...
        try:
            ...
            if init_sol is not None and is_cn(Ma, Cn):
                ...
                for w in deg_rank:
                    ...
                    not_edges = [(w, j) for j in range(graph_size) if Ma[w, j] == 0]
                    ...
                    if diff_edge is not None:
                        ...
                        # Write graph to file
                        write_graph(Ma, diff_edge, "{}/m{}.graph".format(path, z), cn=Cn)
                        Ma_list.append(Ma)
                        z += 1
                        # set_trace()
            ...
            # end if
                ...
                edges = [(i, j) for i in range(graph_size) for j in range(i + 1, graph_size) if Ma[i, j] == 1]
...
def write_graph(Ma, diff_edge, filepath, cn=0):
    with open(filepath, "w") as out:
        ...
        nz_indices_list = list(np.nonzero(Ma))
        leng = len(nz_indices_list[0])//2
        # print ('num of edge')
        # print (len(nz_indices_list[0]))
        for (i, j) in zip(nz_indices_list[0][0:leng], nz_indices_list[1][0:leng]):
            out.write("{} {}\n".format(i, j))
        # end
        out.write('-1\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-samples', default=100, type=int, help='How many samples?')
    parser.add_argument('-path', default='=training', type=str, help='Save path', required=True)
    parser.add_argument('-graph_size', default=15, type=int, help='Number of vertices')
